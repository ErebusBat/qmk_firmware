// Copyright 2025 @ ErebusBat
#include QMK_KEYBOARD_H
#include "erebusbat.h"

enum shift_keycodes {
    U_T_AGCR = SAFE_RANGE, //USB Toggle Automatic GCR control
    DBG_TOG,               //DEBUG Toggle On / Off
    DBG_MTRX,              //DEBUG Toggle Matrix Prints
    DBG_KBD,               //DEBUG Toggle Keyboard Prints
    DBG_MOU,               //DEBUG Toggle Mouse Prints
    MD_BOOT,               //Restart into bootloader after hold timeout
};

#define TG_NKRO MAGIC_TOGGLE_NKRO //Toggle 6KRO / NKRO mode

keymap_config_t keymap_config;

enum layers {
    L_BASE = 0,
    L_FN   = 1,
    L_CFN  = 2,  // Ctrl FN
};

// clang-format off
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {

    /*

     # 0
     # effective_min_key_width=4 token_min_key_width=11
      +----++----+----+----+----+ +----+----+----+----++----+----+----+----++----+  +----+----+----+----+
      |HYPR|| F1 | F2 | F3 | F4 | | F5 | F6 | F7 | F8 || F9 |F10 |F11 |F12 ||DEL |  |HOME|END |PGUP|PGDN|
      +----++----+----+----+----+ +----+----+----+----++----+----+----+----++----+  +----+----+----+----+
      +----+----+----+----+----+----+----+----+----+----+----+----+----+---------+  +----+----+----+----+
      | ~  | 1  | 2  | 3  | 4  | 5  | 6  | 7  | 8  | 9  | 0  | -  | =+ |  BSPC   |  |NLCK|PSLS|PAST|PSCR|
      +----+----+----+----+----+----+----+----+----+----+----+----+----+---------+  +----+----+----+----+
      +-------+----+----+----+----+----+----+----+----+----+----+----+----+-------+ +----+----+----+----+
      |  TAB  | Q  | W  | E  | R  | T  | Y  | U  | I  | O  | P  |LBRC|RBRC| BSLS  | | P7 | P8 | P9 |PMNS|
      +-------+----+----+----+----+----+----+----+----+----+----+----+----+-------+ +----+----+----+----+
      +--------+----+----+----+----+----+----+----+----+----+----+----+----------+  +----+----+----+----+
      |EscCtrl | A  | S  | D  | F  | G  | H  | J  | K  | L  | ;: | '" |   ENT    |  | P4 | P5 | P6 |PPLS|
      +--------+----+----+----+----+----+----+----+----+----+----+----+----------+  +----+----+----+----+
      +----------+----+----+----+----+----+----+----+----+----+----+--------+       +----+----+----+----+
      |   LSFT   | Z  | X  | C  | V  | B  | N  | M  | ,< | .> | /? |  RSFT  |       | P1 | P2 | P3 |PENT|
      +----------+----+----+----+----+----+----+----+----+----+----+--------+       +----+----+----+----+
                                                                             +----+      +----+----+
                                                                             | UP |      | P0 |PDOT|
                                                                             +----+      +----+----+
      +-----+-----++-----+------------------------------+-------+-------+----+----+----+
      | MO2 |LALT ||LGUI |             SPC              | RGUI  |  MO1  |LEFT|DOWN|RGHT|
      +-----+-----++-----+------------------------------+-------+-------+----+----+----+
*/

    [0] = LAYOUT(

     // EB_ESC_HYPR        F1           F2           F3           F4               F5           F6           F7           F8              F9           F10          F11          F12             DEL                 HOME         END          PGUP         PGDN
        EB_ESC_HYPR,    KC_F1,       KC_F2,       KC_F3,       KC_F4,           KC_F5,       KC_F6,       KC_F7,       KC_F8,          KC_F9,       KC_F10,      KC_F11,      KC_F12,         KC_DEL,             KC_HOME,     KC_END,      KC_PGUP,     KC_PGDN,

     // ~               1            2            3            4            5            6            7            8            9            0         -            =+              BSPC                             NLCK         PSLS         PAST         PSCR
        KC_GRV,      KC_1,        KC_2,        KC_3,        KC_4,        KC_5,        KC_6,        KC_7,        KC_8,        KC_9,        KC_0,        KC_MINS,     KC_EQL,      KC_BSPC,                         KC_NLCK,     KC_PSLS,     KC_PAST,     KC_PSCR,

     //    TAB                 Q            W            E            R            T            Y            U            I            O            P            LBRC         RBRC         BSLS                      P7           P8           P9           PMNS
        KC_TAB,             KC_Q,        KC_W,        KC_E,        KC_R,        KC_T,        KC_Y,        KC_U,        KC_I,        KC_O,        KC_P,        KC_LBRC,     KC_RBRC,     KC_BSLS,                  KC_P7,       KC_P8,       KC_P9,       KC_PMNS,

     // EB_CAPS                   A            S            D            F            G            H            J            K            L         ;:           '"              ENT                                 P4           P5           P6           PPLS
        EB_CAPS,               KC_A,        KC_S,        KC_D,        KC_F,        KC_G,        KC_H,        KC_J,        KC_K,        KC_L,        KC_SCLN,     KC_QUOT,     KC_ENT,                             KC_P4,       KC_P5,       KC_P6,       KC_PPLS,

     //    LSFT                         Z            X            C            V            B            N            M         ,<           .>           /?              RSFT                                       P1           P2           P3           PENT
        KC_LSFT,                     KC_Z,        KC_X,        KC_C,        KC_V,        KC_B,        KC_N,        KC_M,        KC_COMM,     KC_DOT,      KC_SLSH,     KC_RSFT,                                   KC_P1,       KC_P2,       KC_P3,       KC_PENT,

     //                                                                                                                                                                                             UP                            P0           PDOT
                                                                                                                                                                                                 KC_UP,                        KC_P0,       KC_PDOT,

     // MO2                LALT             LGUI            SPC                                                                              RGUI             MO1                      LEFT         DOWN         RGHT
        MO(2),          KC_LALT,         KC_LGUI,        KC_SPC,                                                                          KC_RGUI,            MO(1),                KC_LEFT,     KC_DOWN,     KC_RGHT),

/*
     # 0
     # effective_min_key_width=10 token_min_key_width=10 passed_min_key_width=10
     +----------+  +----------+----------+----------+----------+  +----------+----------+----------+----------+ +----------+----------+----------+----------+  +----------+     +----------+----------+----------+----------+
     |   ESC    |  |    F1    |    F2    |    F3    |    F4    |  |    F5    |    F6    |    F7    |    F8    | |    F9    |   F10    |   F11    |   F12    |  |   DEL    |     |   HOME   |   END    |   PGUP   |   PGDN   |
     +----------+  +----------+----------+----------+----------+  +----------+----------+----------+----------+ +----------+----------+----------+----------+  +----------+     +----------+----------+----------+----------+
     +----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+---------------------+     +----------+----------+----------+----------+
     |    ~     |    1     |    2     |    3     |    4     |    5     |    6     |    7     |    8     |    9     |    0     |    -     |    =+    |        BSPC         |     |   NLCK   |   PSLS   |   PAST   |   PSCR   |
     +----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+---------------------+     +----------+----------+----------+----------+
     +----------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------------+    +----------+----------+----------+----------+
     |      TAB       |    Q     |    W     |    E     |    R     |    T     |    Y     |    U     |    I     |    O     |    P     |   LBRC   |   RBRC   |      BSLS      |    |    P7    |    P8    |    P9    |   PMNS   |
     +----------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------------+    +----------+----------+----------+----------+
     +------------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+------------------------+     +----------+----------+----------+----------+
     |     EB_CAPS      |    A     |    S     |    D     |    F     |    G     |    H     |    J     |    K     |    L     |    ;:    |    '"    |          ENT           |     |    P4    |    P5    |    P6    |   PPLS   |
     +------------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+------------------------+     +----------+----------+----------+----------+
     +------------------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+------------------+                +----------+----------+----------+----------+
     |          LSFT          |    Z     |    X     |    C     |    V     |    B     |    N     |    M     |    ,<    |    .>    |    /?    |       RSFT       |                |    P1    |    P2    |    P3    |    UP    |
     +------------------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+------------------+                +----------+----------+----------+----------+
                                                                                                                                                                  +----------+
                                                                                                                                                                  |   PENT   |
                                                                                                                                                                  +----------+
     +-------------+-------------+------------+--------------------------------------------------------------------+----------------+----------------+                                     +----------+----------+
     |    LCTL     |    LALT     |    LGUI    |                                SPC                                 |      RGUI      |      MO1       |                                     |    P0    |   PDOT   |
     +-------------+-------------+------------+--------------------------------------------------------------------+----------------+----------------+                                     +----------+----------+
                                                                                                                                                       +----------+----------+----------+
                                                                                                                                                       |   LEFT   |   DOWN   |   RGHT   |
                                                                                                                                                       +----------+----------+----------+
*/


    [1] = LAYOUT(

     // EB_ESC_HYPR      F1        F2        F3        F4          F5        F6        F7        F8           F9        F10       F11       F12         DEL            HOME      END       PGUP      PGDN
        _______,     _______,  _______,  _______,  _______,    _______,  _______,  _______,  _______,     _______,  _______,  _______,  _______,    KC_INS,        KC_MPLY,  KC_MSTP,  KC_VOLU,  KC_MUTE,

     // ~            1         2         3         4         5         6         7         8         9         0      -         =+           BSPC                     NLCK      PSLS      PAST      PSCR
        _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,                 KC_MPRV,  KC_MNXT,  KC_VOLD,  KC_SLCK,

     //    TAB            Q         W         E         R         T         Y         U         I         O         P         LBRC      RBRC      BSLS                P7        P8        P9        PMNS
        _______,       RGB_SPD,  RGB_VAI,  RGB_SPI,  RGB_HUI,  RGB_SAI,  _______,  _______,  U_T_AGCR, _______,  _______,  _______,  _______,  _______,            _______,  _______,  _______,  _______,

     // EB_CAPS              A         S         D         F         G         H         J         K         L      ;:        '"           ENT                        P4        P5        P6        PPLS
        _______,          RGB_RMOD, RGB_VAD,  RGB_MOD,  RGB_HUD,  RGB_SAD,  _______,  _______,  _______,  _______,  _______,  _______,  _______,                   _______,  _______,  _______,  _______,

     //    LSFT                   Z         X         C         V         B         N         M      ,<        .>        /?           RSFT                            P1        P2        P3        PENT
        _______,               RGB_TOG,  _______,  _______,  _______,  MD_BOOT,  TG_NKRO,  _______,  _______,  _______,  _______,  _______,                        _______,  _______,  _______,  _______,

     //                                                                                                                                                   UP                    P0        PDOT
                                                                                                                                                       _______,              _______,  _______,

     // MO2             LALT        LGUI         SPC                                                           RGUI        MO1                  LEFT      DOWN      RGHT
        _______,     _______,    _______,     _______,                                                      KC_APP,        _______,          _______,  _______,  _______),

/*

     # 1
     # effective_min_key_width=8 token_min_key_width=8
      +--------+ +--------+--------+--------+--------+  +--------+--------+--------+--------+ +--------+--------+--------+--------+ +--------+    +--------+--------+--------+--------+
      |        | |        |        |        |        |  |        |        |        |        | |        |        |        |        | |  INS   |    |  PLAY  |  STOP  |  VOL+  |  MUTE  |
      +--------+ +--------+--------+--------+--------+  +--------+--------+--------+--------+ +--------+--------+--------+--------+ +--------+    +--------+--------+--------+--------+
      +--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+-----------------+    +--------+--------+--------+--------+
      |        |        |        |        |        |        |        |        |        |        |        |        |        |                 |    |  PREV  |  NEXT  |  VOL-  |  SLCK  |
      +--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+-----------------+    +--------+--------+--------+--------+
      +-------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+-------------+   +--------+--------+--------+--------+
      |             | 💡SPD  | 💡VAI  | 💡SPI  | 💡HUI  | 💡SAI  |        |        |U_T_AGCR|        |        |        |        |             |   |        |        |        |        |
      +-------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+-------------+   +--------+--------+--------+--------+
      +---------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+-------------------+    +--------+--------+--------+--------+
      |               | 💡RMOD | 💡VAD  | 💡MOD  | 💡HUD  | 💡SAD  |        |        |        |        |        |        |                   |    |        |        |        |        |
      +---------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+-------------------+    +--------+--------+--------+--------+
      +-------------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+---------------+             +--------+--------+--------+--------+
      |                   | 💡TOG  |        |        |        |MD_BOOT |TG_NKRO |        |        |        |        |               |             |        |        |        |        |
      +-------------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+---------------+             +--------+--------+--------+--------+
                                                                                                                                      +--------+           +--------+--------+
                                                                                                                                      |        |           |        |        |
                                                                                                                                      +--------+           +--------+--------+
      +----------+----------++----------+-------------------------------------------------------+-------------+-------------++--------+--------+--------+
      |          |          ||          |                                                       |     APP     |             ||        |        |        |
      +----------+----------++----------+-------------------------------------------------------+-------------+-------------++--------+--------+--------+
*/

/*

     # 2
     # effective_min_key_width=10 token_min_key_width=10
      +----------+  +----------+----------+----------+----------+  +----------+----------+----------+----------+ +----------+----------+----------+----------+  +----------+     +----------+----------+----------+----------+
      |          |  |          |          |   F13    |   F14    |  |   F15    |   F16    |   F17    |   F19    | |   F20    |   F21    |   F22    |          |  |EB_KC_DICT|     |          |          |          |          |
      +----------+  +----------+----------+----------+----------+  +----------+----------+----------+----------+ +----------+----------+----------+----------+  +----------+     +----------+----------+----------+----------+
      +----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+---------------------+     +----------+----------+----------+----------+
      |          |          |          |          |          |          |          |          |          |          |          |          |          |                     |     |          |          |          |          |
      +----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+---------------------+     +----------+----------+----------+----------+
      +----------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------------+    +----------+----------+----------+----------+
      |                |          |          |          |          |          |          |          |          |          |          |          |          |                |    |          |          |          |          |
      +----------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------------+    +----------+----------+----------+----------+
      +------------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+------------------------+     +----------+----------+----------+----------+
      |                  |          |          |          |          |          |   LEFT   |   DOWN   |    UP    |   RGHT   |          |          |       EB_TUI_NL        |     |          |          |          |          |
      +------------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+------------------------+     +----------+----------+----------+----------+
      +------------------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+------------------+                +----------+----------+----------+----------+
      |                        |          |          |          |          |          |          |          |          |          |          |                  |                |          |          |          |          |
      +------------------------+----------+----------+----------+----------+----------+----------+----------+----------+----------+----------+------------------+                +----------+----------+----------+----------+
                                                                                                                                                                   +----------+             +----------+----------+
                                                                                                                                                                   |          |             |          |          |
                                                                                                                                                                   +----------+             +----------+----------+
      +-------------+-------------+------------+--------------------------------------------------------------------+----------------+----------------+ +----------+----------+----------+
      | C(KC_LSFT)  |             |            |                                                                    |                |                | |          |          |          |
      +-------------+-------------+------------+--------------------------------------------------------------------+----------------+----------------+ +----------+----------+----------+
*/

    [2] = LAYOUT(

     // EB_ESC_HYPR       F1          F2          F3          F4             F5          F6          F7          F8             F9          F10         F11         F12            DEL               HOME        END         PGUP        PGDN
        _______,       _______,    _______,    KC_F13,     KC_F14,        KC_F15,     KC_F16,     KC_F17,     KC_F19,        KC_F20,     KC_F21,     KC_F22,     _______,       EB_KC_DICT,       _______,    _______,    _______,    _______,

     // ~              1           2           3           4           5           6           7           8           9           0        -           =+             BSPC                          NLCK        PSLS        PAST        PSCR
        _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,                      _______,    _______,    _______,    _______,

     //    TAB               Q           W           E           R           T           Y           U           I           O           P           LBRC        RBRC        BSLS                    P7          P8          P9          PMNS
        _______,          _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,                _______,    _______,    _______,    _______,

     // EB_CAPS                 A           S           D           F           G           H           J           K           L        ;:          '"             ENT                              P4          P5          P6          PPLS
        _______,             _______,    _______,    _______,    _______,    _______,    KC_LEFT,    KC_DOWN,    KC_UP,      KC_RGHT,    _______,    _______,    EB_TUI_NL,                       _______,    _______,    _______,    _______,

     //    LSFT                       Z           X           C           V           B           N           M        ,<          .>          /?             RSFT                                   P1          P2          P3          PENT
        _______,                   _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,                               _______,    _______,    _______,    _______,

     //                                                                                                                                                                               UP                         P0          PDOT
                                                                                                                                                                                   _______,                   _______,    _______,

     // MO2               LALT           LGUI           SPC                                                                        RGUI           MO1                     LEFT        DOWN        RGHT
        C(KC_LSFT),    _______,       _______,       _______,                                                                   _______,          _______,             _______,    _______,    _______),



    /*
    [X] = LAYOUT(
        _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,_______,_______,_______,           _______, _______, _______, _______, \
        _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,          _______, _______, _______, _______, \
        _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,          _______, _______, _______, _______, \
        _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,                   _______, _______, _______, _______, \
        _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,                            _______, _______, _______,          \
                                                                                                                             _______,                                     _______, \
        _______, _______, _______,                   _______,                            _______, _______,                                     _______, _______,                   \
                                                                                                                    _______, _______, _______                                      \
    ),
    */
};
// clang-format on

void matrix_init_user(void) {
};

void matrix_scan_user(void) {
};

#define MODS_SHIFT (get_mods() & MOD_BIT(KC_LSHIFT) || get_mods() & MOD_BIT(KC_RSHIFT))
#define MODS_CTRL (get_mods() & MOD_BIT(KC_LCTL) || get_mods() & MOD_BIT(KC_RCTRL))
#define MODS_ALT (get_mods() & MOD_BIT(KC_LALT) || get_mods() & MOD_BIT(KC_RALT))

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    static uint32_t key_timer;

    switch (keycode) {
        case U_T_AGCR:
            if (record->event.pressed && MODS_SHIFT && MODS_CTRL) {
                TOGGLE_FLAG_AND_PRINT(usb_gcr_auto, "USB GCR auto mode");
            }
            return false;
        case DBG_TOG:
            if (record->event.pressed) {
                TOGGLE_FLAG_AND_PRINT(debug_enable, "Debug mode");
            }
            return false;
        case DBG_MTRX:
            if (record->event.pressed) {
                TOGGLE_FLAG_AND_PRINT(debug_matrix, "Debug matrix");
            }
            return false;
        case DBG_KBD:
            if (record->event.pressed) {
                TOGGLE_FLAG_AND_PRINT(debug_keyboard, "Debug keyboard");
            }
            return false;
        case DBG_MOU:
            if (record->event.pressed) {
                TOGGLE_FLAG_AND_PRINT(debug_mouse, "Debug mouse");
            }
            return false;
        case MD_BOOT:
            if (record->event.pressed) {
                key_timer = timer_read32();
            } else {
                if (timer_elapsed32(key_timer) >= BOOTKEY_HOLD_MS) {
                    reset_keyboard();
                }
            }
            return false;
        default:
            return true; //Process all other keycodes normally
    }
}
