...
This is ErebusBat's fork of the QMK firmware repository with custom keymaps for multiple keyboards. It tracks four upstreams:
...
- **massdrop** — `https://github.com/Massdrop/qmk_firmware.git` (Drop/Massdrop boards — ARM_ATSAM)
...
| `drop_shift_10key` | massdrop/master | Drop Shift v1 (compact 1800 / 10-key layout) |
...
After updating a shared file on any branch, sync it to the other branches (`playground`, `k2_he_2025q3`, `hall_effect_playground`, `master`, `erebusbat-keyboard`, `drop_shift_10key`):
...
**Drop Shift v1** (branch: `drop_shift_10key`):
```bash
qmk compile -kb massdrop/shift -km erebusbat
# Produces: .build/massdrop_shift_erebusbat.bin
```

> **NOTE:** `qmk flash` does NOT work for the Drop Shift v1. Use `mdloader` instead (see below).

#### Flashing the Drop Shift v1
...
The Drop Shift v1 uses Massdrop's custom `ARM_ATSAM` HAL — **not** standard QMK ARM. Mainstream QMK removed `arm_atsam` support in November 2024. The `drop_shift_10key` branch must track `massdrop/master` (the Massdrop fork) — do not rebase onto upstream QMK.
